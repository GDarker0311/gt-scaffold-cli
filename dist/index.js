...
function modify (obj, updateQuery) {
  var keys = Object.keys(updateQuery)
    , firstChars = underscore$1.map(keys, function (item) { return item[0]; })
    , dollarFirstChars = underscore$1.filter(firstChars, function (c) { return c === '$'; })
    , newDoc, modifiers
    ;
  ...
  if (dollarFirstChars.length === 0) {
    ...
    // Apply modifiers
    modifiers = underscore$1.uniq(keys);
    newDoc = deepCopy(obj);
...
logicalOperators.$where = function (obj, fn) {
  ...
  if (!underscore$1.isFunction(fn)) { throw new Error("$where operator used without a function"); }

  result = fn.call(obj);
  if (!underscore$1.isBoolean(result)) { throw new Error("$where function must return boolean"); }
...
var checkObject_1 = checkObject;
var isPrimitiveType_1 = isPrimitiveType;
var modify_1 = modify;
var getDotValue_1 = getDotValue;
var match_1 = match;
var areThingsEqual_1 = areThingsEqual;
var compareThings_1 = compareThings;
...
function getRandomArray (n) {
  ...
  return res;
}var getRandomArray_1 = getRandomArray;
...
var defaultCompareKeysFunction_1 = defaultCompareKeysFunction;
...
Index.prototype.insert = function (doc) {
  ...
  if (!util.isArray(key)) {
    ...
    keys = underscore$1.uniq(key, projectForUnique);
...
Index.prototype.remove = function (doc) {
  ...
  if (!util.isArray(key)) {
    ...
    underscore$1.uniq(key, projectForUnique).forEach(function (_key) {
...
function mkdirP (p, opts, f, made) {
    ...
    xfs.mkdir(p, mode, function (er) {
        ...
        switch (er.code) {
            case 'ENOENT':
                if (path__default.dirname(p) === p) return cb(er);
...
Persistence.getNWAppFilename = function (appName, relativeFilename) {
  ...
  switch (process.platform) {
    ...
    case 'win64':
      ...
      home = path__default.join(home, appName);
      break;
    case 'darwin':
      home = process.env.HOME;
      if (!home) { throw new Error("Couldn't find the base application data directory"); }
      home = path__default.join(home, 'Library', 'Application Support', appName);
      break;
    case 'linux':
      home = process.env.HOME;
      if (!home) { throw new Error("Couldn't find the base application data directory"); }
      home = path__default.join(home, '.config', appName);
      break;
    default:
      throw new Error("Can't use the Node Webkit relative path for platform " + process.platform);
  }

  return path__default.join(home, 'nedb-data', relativeFilename);
};
...
Cursor.prototype.project = function (candidates) {
  ...
  this._projection = underscore$1.omit(this._projection, '_id');
...
Datastore.prototype.getCandidates = function (query, dontExpireStaleDocs, callback) {
  ...
  function (cb) {
    ...
    usableQueryKeys = underscore$1.intersection(usableQueryKeys, indexNames);
...
Datastore.prototype._update = function (query, updateQuery, options, cb) {
  ...
  , function () {   // Perform the update
    ...
    self.getCandidates(query, function (err, candidates) {
      ...
      // Update the datafile
      var updatedDocs = underscore$1.pluck(modifications, 'newDoc');
